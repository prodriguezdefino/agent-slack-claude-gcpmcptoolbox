...
data "archive_file" "app_src_archive" {
  type        = "zip"
  source_dir  = "../src" # Archive the src directory
  output_path = ".terraform/temp/app_src.zip" // Temporary path for the archive

  # Adding pom.xml to the trigger hash.
  # Since source_file can only take one file, we use source_dir for src
  # and rely on another trigger for pom.xml if needed, or include it in the archive
  # For simplicity, we'll primarily rely on src dir changes.
  # A more robust way would be to hash multiple specific files/dirs and combine them.
  # For now, hashing the archive of 'src' and adding pom.xml's hash to the null_resource trigger.
}
...
output "cloud_source_repository_url" {
